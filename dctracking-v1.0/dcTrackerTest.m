...
addpath(genpath(fullfile('gco-v3.0')));
...
% fill options struct
opt=getDCOptionsTest;

% fill scene info
sceneInfo=getSceneInfoDCTest;

frames=1:length(sceneInfo.frameNums);
% frames=1:431; % do a part of the whole sequence
...
for ekfexp=1:5
    mhsekf=getSplinesFromEKF(fullfile('test','ekf',sprintf('e%04d.mat',ekfexp)),frames,alldpoints,T);
    mhs=[mhs mhsekf];
end
...
% nAddRandomModels=1; % random models
% nAddModelsOutliers=1;
