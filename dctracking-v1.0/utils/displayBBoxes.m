function displayBBoxes(sceneInfo,frameNums,X,Y,W,H,recog_result,options)
...
stateInfo.visits = zeros(F,I);
stateInfo.passed = zeros(F,I);
...
if options.drawLocations
    locationsImg=double(imread([sceneInfo.imgFolder ...
        sprintf(sceneInfo.imgFileFormat,0)]))/255;
    locations = size(options.B);
    for i=1:locations
        locationsImg = insertObjectAnnotation(locationsImg, ...
            'Rectangle', options.B(i,:), num2str(i), 'Color', 'red', ...
            'FontSize',18);
    end
    imwrite(locationsImg, fullfile(options.outFolder,'locations.jpg'));
end
...
for t=1:F
    ...
    if options.displayRecogResult
        for id=extar
            tx=X(t,id);
            ty=Y(t,id)-H(t,id)*1/3; % inside
            %             ty=Y(t,id)-H(t,id)-10; % on top
            text(tx,ty,recog_result(id),'color',getColorFromID(id), ...
                'HorizontalAlignment','center', ...
                'FontSize',W(t,id)/6, 'FontUnits','pixels','FontWeight','bold');
        end
    end
    ...
    % Visits
    if options.displayVisits
        for id=extar
            tx=X(t,id);
            ty=Y(t,id)+H(t,id)*1/3; % below
            ty2=Y(t,id)+H(t,id)*2/3; % below
            P = [X(t,id) Y(t,id)];
            locationsCount = 0;
            for i=1:options.numLocations
                if PinsideB(P,options.B(i,:))
                    locationsCount = locationsCount + 1;
                    location = i;
                end
            end
            if locationsCount ==1
                stateInfo.passed(t,id)=location;
                text(tx,ty,sprintf('in location %i',location),'color',getColorFromID(id), ...
                    'HorizontalAlignment','center', ...
                    'FontSize',W(t,id)/6, 'FontUnits','pixels','FontWeight','bold');
                stationary = 1;
                for ii=1:options.frames2beStationary-1
                    pP = [X(t-ii,id) Y(t-ii,id)];%previous points
                    if (PinsideB(pP,options.B(location,:))),
                        stationary = stationary + 1;
                    end
                end
                if stationary == options.frames2beStationary
                    stateInfo.visits(t,id)=location;
                    text(tx,ty2,sprintf('stationary in %i',location),'color',getColorFromID(id), ...
                        'HorizontalAlignment','center', ...
                        'FontSize',W(t,id)/6, 'FontUnits','pixels','FontWeight','bold');
                end
            end
...
if options.logVisits
    fileID = fopen('test\Results\visits.txt','w');
    for i=1:I
        startedId = 0;
        visitLoc = 0;
        for ii=1:F
            if stateInfo.visits(ii,i) ~= 0 && ...
                    stateInfo.visits(ii,i) ~= visitLoc
                visitLoc = stateInfo.visits(ii,i);
                if startedId == 0
                    startedId = 1;
                    fprintf(fileID,'Trajectory %i visited locations: %i', ...
                        i,visitLoc);
                else
                    fprintf(fileID,' and then %i',visitLoc);
                end
            end
        end
        if startedId ~= 0
...
if options.logPasses
    fileID = fopen('test\Results\passes.txt','w');
    for i=1:I
        startedId = 0;
        passedLoc = 0;
        for ii=1:F
            if stateInfo.passed(ii,i) ~= 0 && ...
                    stateInfo.passed(ii,i) ~= passedLoc
                passedLoc = stateInfo.passed(ii,i);
                if startedId == 0
                    startedId = 1;
                    fprintf(fileID,'Trajectory %i passed locations: %i', ...
                        i,passedLoc);
                else
                    fprintf(fileID,' and then %i',passedLoc);
                end
            end
        end
        if startedId ~= 0
...
if options.logRecogResults
    fileID = fopen('test\Results\FR.txt','w');
    for i=1:I
        fprintf(fileID,'Trajectory %i face recognition result: %s\n',i,recog_result{i});
    end
    fclose(fileID);
end