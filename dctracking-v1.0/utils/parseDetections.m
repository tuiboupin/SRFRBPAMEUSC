...
elseif detFileType==2
    ...
    for t=frToParse
        ...
        boxtop=zeros(1,nboxes);
        scores=zeros(1,nboxes);
        recog_result=cell(1,nboxes);
        recog_positive=cell(1,nboxes);
        ...
        for i=0:Nt-1
            % score
            boxid=i+1;
            objAttributes = objects.item(i).getAttributes;
            ...
            if objAttributes.getLength == 4
               recog_result(boxid) = objects.item(i).getAttribute('recog_result');
               recog_positive(boxid) = objects.item(i).getAttribute('recog_over_threshold');
            end
        ...
        detections(t).wd=widths;
        detections(t).sc=scores;
        detections(t).recog_result=recog_result;
        detections(t).recog_positive=recog_positive;
