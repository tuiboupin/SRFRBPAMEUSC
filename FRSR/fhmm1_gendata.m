function [ myDatabase,minmax ] = fhmm1_gendata( folder, file_type, noc, fhmm1_path, DB_file )
%UNTITLED5 Summary of this function goes here
%   Detailed explanation goes here

fprintf('START fhmm1_gendata\n');
...
data_folder_contents = dir (folder);
myDatabase = cell(0,0);
person_index = 0;
max_coeffs = [-Inf -Inf -Inf];
min_coeffs = [ Inf  0  0];

I = imread([folder '\s1\1.jpg']);
[r c] = size(I);
...
%     person_folder_contents = dir(['./data/',person_name,'/*.pgm']);    
    person_folder_contents = dir([folder,'\',person_name,'\',file_type]);    
    blk_cell = cell(0,0);
    for face_index=1:5
        I = imread([folder,'\',person_name,'\',person_folder_contents(ufft(face_index),1).name]);
%         I = imresize(I,[56 46]);
...
fprintf('done.\n');
save(DB_file,'myDatabase','minmax');
cd(current_path);
end
